...
(function() {
   ...
   connect.core.initCCP = function(containerDiv, paramsIn) {
      ...
      connect.core.getEventBus().subscribe(connect.EventType.ACK_TIMEOUT, function() {
         ...
         if (params.loginPopup !== false) {
            try {
              ...
              // connect.getLog().warn("ACK_TIMEOUT occurred, attempting to pop the login page if not already open.");
               ...
               //opens model window
             // $('#preLoadingModalCenter').modal({backdrop: 'static', keyboard: false});
              ...
              //       logg("WARN Popup window blocked");
              //    // model updates when popup is blocked
              //   $('#preLoadingModalCenter').on('shown.bs.modal', function (event) {
              //     var modal = $(this)
              //     modal.find('.body-content').html('<h3>Popup Blocked</h3><p>Disable your pop-up blocker and hit refresh</p><button type="button" onclick="window.location.reload(true)" class="btn btn-info">Refresh</button>');
              //   })
              ...
              // $('#preLoadingModalCenter').on('shown.bs.modal', function (event) {
              //   var modal = $(this)
              //   modal.find('.body-content').html('<h3>Network Timeout</h3><p>Please refresh the page</p>');
              // })
		...
		//console.log(connect.core.iframeRefreshInterval)
         if (connect.core.iframeRefreshInterval == null) {
            ...
            conduit.onUpstream(connect.EventType.ACKNOWLEDGE, function() {    
             // $('#preLoadingModalCenter').modal('hide'); 
               my_window.close();
               this.unsubscribe();
